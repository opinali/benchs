/**
 * "Easy" test: Uses only compile-time constants for loop bounds, making things easier for loop
 * optimizations reliant on static analysis.
 *
 * (c) 2002 by Osvaldo Pinali Doederlein. Released to public domain.
 */
public class MethodEasy extends MethodBench {
  /**
   * Declare the benchmark instance here, init later, to fight dataflow optimizations.
   */
  static MethodEasy t;
  ...
  final void benchInterface() {
    final int calls = CALLS / 2;

    final HelperInterface helper1 = Integer.parseInt("1") == 1 ? (HelperInterface) new HelperB()
        : (HelperInterface) new HelperD();

    for (int i = ZERO; i < calls; ++i)
      helper1.polyHardF(i);

    final HelperInterface helper2 = Integer.parseInt("0") == 1 ? (HelperInterface) new HelperB()
        : (HelperInterface) new HelperD();
  ...
  final void benchTrulyPoly() {
    ...
    final HelperC h1 = new HelperC(), h2 = new HelperD();
